...
pc_expr <- read.table('quantification/NHP_macaque/pc_expression_NHP_macaque.tsv', header = T, row.names = 1, sep = '\t')
pgene_expr <- pgene_expr[rowSums(pgene_expr > 0.5) >= 100,] # threshold 0.5
pc_expr <- pc_expr[rowSums(pc_expr > 1) >= 100, ] # threshold 1
expr <- rbind(pgene_expr, pc_expr)

colNames <- gsub('mRNA_ployA_.*_','',colnames(expr), perl = T)
...
WGCNA_acrossRegions <- function(expression, region, species = 'MM'){
  expr_df <- expression %>% dplyr::select(ends_with(region))
  ...
  # Outlier dectection
  pdf(paste0('WGCNA/macaque/tree/', region, '.pdf'), width = 10, height = 10)
  ...
  net <- blockwiseModules(dataExpr, power = 20, maxBlockSize = nGenes,
                          ...
                          mergeCutHeight = 0.25, detectCutHeight=0.995, 
                          minCoreKME=0.7, minCoreKMESize=3, minKMEtoStay=0.7,
                          numericLabels = TRUE, pamRespectsDendro = FALSE,
                          saveTOMs=TRUE,
                          saveTOMFileBase = paste0('WGCNA/macaque/tom/', region),
                          verbose = 3, nThreads = 16)
  # WGCNA plot
  moduleLabels <- net$colors
  moduleColors <- labels2colors(moduleLabels)
  pdf(paste0('WGCNA/macaque/plot/', region, '.pdf'), width = 20, height = 20)
...
saveRDS(res, 'WGCNA/macaque_res.rds')