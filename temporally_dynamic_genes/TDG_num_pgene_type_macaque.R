unitary_pgene <- read.table('parent/fasta/unitary_pgene.bed', sep = '\t', header = F) %>% select(V28) %>% pull()
duplicated_pgene <- read.table('parent/fasta/duplicated_pgene.bed', sep = '\t', header = F) %>% select(V28) %>% pull()
processed_pgene <-  read.table('parent/fasta/processed_pgene.bed', sep = '\t', header = F) %>% select(V28) %>% pull()
...
ggsave('temporal_dynamic_expression/human_vs._macaque/subtype/pgene_#macaque.pdf', p, width = 10, height = 5)
